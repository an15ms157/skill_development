...
factor = 0.15  # Set catchup factor
...
# Only invest when SMA_50 < SMA_200 (reverse crossover)
sma1 = 'SMA_50'
sma2 = 'SMA_200'
...
for month in months:
    ...
    # Check if we can invest this month (reverse crossover)
...
plt.title(f'{TICKER} Reverse Crossover SIP: Capital Invested, Net Valuation, Savings, and ROIC Over Time')
ax2.grid(True, which='both', axis='x', linestyle='--', alpha=0.5)
fig.tight_layout()
plt.savefig(f'plots/strategy/{TICKER}_reverse_crossover_sip_simulation.png')
plt.show()
...
with open(f'data/{TICKER}_reverse_crossover_sip_simulation_timeseries.json', 'w') as f:
