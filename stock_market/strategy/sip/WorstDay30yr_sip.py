...
def perform_sip_simulation(df, rolling_years=30):
    """
    Perform a 30-year rolling window SIP simulation by investing 1 USD on the worst trading day (lowest index value) each month.
    Returns a list of dictionaries containing ROIC results for each window.
    """
    print("Starting 30-Year Worst Day SIP Simulation (Investing on the day with the lowest index value each month)...")
    ...
    for start_year in range(min_year, max_year + 1):
        ...
        for _, group in grouped:
            if not group.empty:
                # Find the day with the lowest index value (worst day)
                worst_idx = group['Index'].idxmin()
                investment_dates.append(group.loc[worst_idx, 'Date'])
                investment_prices.append(group.loc[worst_idx, 'Index'])
...
def get_plot_filenames():
    ...
    return {
        'plot_filename': '30yr_worst_day_sip_subplot.png',
        'csv_filename': 'roic_vs_year_worst_day.csv'
    }
